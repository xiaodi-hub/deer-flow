...
- Tools loaded via `get_available_tools()` - combines sandbox, built-in, MCP, community, and subagent tools; custom agent profiles may whitelist `tool_groups`, `mcp_servers`, and `mcp_tools`
- System prompt generated by `apply_prompt_template()` with skills, memory, and subagent instructions
- Custom agent profiles live under `{base_dir}/users/{user_id}/agents/{agent_name}/config.yaml` + `SOUL.md`. Besides the runtime fields (`model`, `tool_groups`, `skills`), profiles carry gallery metadata (`display_name`, `category`, `icon`, `tags`, `starter_prompts`, `enabled`), MCP allowlists, memory policy, and workspace policy. Gateway `assistant_id=<agent-name>` and frontend agent chats both route through the same `lead_agent` factory with `agent_name` injected.
...
4. **ThreadDataMiddleware** - Creates per-thread directories under the user's isolation scope (`backend/.deer-flow/users/{user_id}/threads/{thread_id}/user-data/{workspace,uploads,outputs}`); resolves `user_id` via `get_effective_user_id()` (falls back to `"default"` in no-auth mode). Custom agents may override the `workspace` mapping through `config.yaml -> workspace` so `/mnt/user-data/workspace` resolves to an agent-shared or custom host directory while uploads/outputs stay thread-owned
...
22. **MemoryMiddleware** - Queues conversations for async memory update (filters to user + final AI responses). Custom agents default to reading both global user memory and per-agent memory; their `memory.write.default` policy controls whether automatic updates write to the agent bucket or the global bucket
...
captures a pre-run and post-run snapshot of the active agent workspace (default
thread workspace, or the custom agent workspace policy) and thread-owned
`outputs` directory. `runtime/runs/worker.py` performs the filesystem scan via
