#!/usr/bin/env bash
# Check the Helm chart's embedded config_version is not behind config.example/app.yaml.
#
# The chart's `config:` block in deploy/helm/deer-flow/values.yaml embeds a
# config_version that must not lag config.example/app.yaml. A stale version is
...
EXAMPLE_YAML="$ROOT/config.example/app.yaml"
...
printf 'config.example/app.yaml config_version=%s\n' "$example"
...
if [ "$chart" -lt "$example" ]; then
  echo "::error::chart config_version ($chart) is behind config.example/app.yaml ($example). Bump 'config_version' in deploy/helm/deer-flow/values.yaml (and the README example) to $example." >&2
  exit 1
fi

echo "OK - chart config_version ($chart) is current with config.example/app.yaml ($example)."
