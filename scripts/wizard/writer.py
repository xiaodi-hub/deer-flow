"""Config directory writer for the Setup Wizard.

Writes config/ as a minimal working configuration and updates .env
...
import yaml
from split_config import CONFIG_GROUPS, _render_file, load_yaml
...
def build_minimal_config(
    *,
    ...
    """Build the content of a minimal merged config mapping."""
    ...
    header = (
        ...
        f"# Full reference: config.example/\n\n"
    )
...
def write_config_yaml(
    ...
    *,
    ...
    """Write (or overwrite) config/ with a minimal working configuration."""
    # Read config_version from config.example/app.yaml if present.
    config_version = 5
    example_path = config_path.parent / "config.example"
    if example_path.exists():
        try:
            raw: dict[str, Any] = {}
            for file_name, _ in CONFIG_GROUPS:
                raw.update(load_yaml(example_path / file_name))
    ...
    for file_name, keys in CONFIG_GROUPS:
        ...
        if group_data:
            (config_path / file_name).write_text(_render_file(file_name, group_data), encoding="utf-8")

    extra_data = {key: value for key, value in data.items() if key not in consumed}
    if extra_data:
        (config_path / "extra.yaml").write_text(_render_file("app.yaml", extra_data), encoding="utf-8")

    mcp_template = example_path / "mcp.yaml"
    if mcp_template.exists():
        (config_path / "mcp.yaml").write_text(mcp_template.read_text(encoding="utf-8"), encoding="utf-8")
